...
export interface SongbookConfig {
    ...
    songbookSideBySide: string;
}
export interface SongbookData {
    songs: Song[];
    config: SongbookConfig;
    setName: string;
    otherSongs: SongShortData[]; // Related songs etc that shouldnt be in by default but may be needed for titles etc
    translationMap: Record<number, Song>; // For side-by-side translations, the map of original song ID to translation song
}
...
function getOriginalSong(song: Song): SongShortData | undefined {
...
function getOriginalSongTitle(song: Song): string[] {
    ...
    const sidebyside = currentData!.config.songbookSideBySide ? currentData!.translationMap[song.id] : undefined;
    if (sidebyside && sidebyside.id === original_song.id) return []; // Already shown as side-by-side so no need to repeat
...
function renderSongColumn(song: Song, index: number | null, config: SongbookConfig): HTMLElement {
    ...
    title.innerHTML = index !== null ? `<span class="song-number">${index + 1}.</span> ${escapeHtml(song.title)}` : escapeHtml(song.title);
    ...
    column.appendChild(renderSongContent(song, config));

    return column;
}

function renderSong(song: Song, index: number, config: SongbookConfig): HTMLElement {
    const sidebyside = config.songbookSideBySide ? currentData!.translationMap[song.id] : undefined;
    ...
    if (sidebyside) {
        div.classList.add('song-sidebyside');
        div.appendChild(renderSongColumn(song, index, config));
        div.appendChild(renderSongColumn(sidebyside, index, config));
    } else {
        div.appendChild(renderSongColumn(song, index, config));
    }
...
function buildMainIndex(songs: Song[], config: SongbookConfig): LanguageIndex[] {
    ...
    songs.forEach((song, idx) => {
        ...
        // If translations are also included. add them to an index also
        const tsong = config.songbookSideBySide ? currentData!.translationMap[song.id] : undefined;
        if (tsong) {
            const tlang = tsong.lang;
            if (!languageGroups.has(tlang)) languageGroups.set(tlang, []);

            [...tsong.alternative_titles, tsong.title]
                .filter((title) => !!title)
                .forEach((title) => {
                    languageGroups.get(tlang)!.push({
                        id: song.id,
                        title,
                        songbookPosition: idx,
                        lang: tlang,
...
function renderMainIndex(songs: Song[], config: SongbookConfig): HTMLElement {
    const indexes = buildMainIndex(songs, config);
...
async function renderSongbook(data: SongbookData): Promise<void> {
    ...
    songbookEl.appendChild(renderMainIndex(data.songs, data.config));
