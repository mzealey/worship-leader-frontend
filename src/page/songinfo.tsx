...
export const PageSongInfo: ComponentType<PageSongInfoProps> = ({ requested_song_id, set_id }: PageSongInfoProps) => {
    ...
    const [related_songs, setRelatedSongs] = useState<SongShortData[]>([]);
    ...
    const load_song = useCallback(
        async (song_id: number) => {
            ...
            try {
                ...
                const _songs = await db.get_songs(song!.related_songs.map((s) => s.id));
                const songs: (SongShortData & { _lang_txt?: string })[] = _songs;
    ...
    const refresh_song = useCallback(async () => {
        if (requested_song_id) {
            ...
            if (db instanceof OfflineDBCommon) await db.refresh_song_from_db(requested_song_id);
            await load_song(requested_song_id);
        }
