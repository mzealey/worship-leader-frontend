...
export const SongXMLDisplay: ComponentType<SongXMLDisplayProps> = ({ song, transpose, no_chords, in_presentation }: SongXMLDisplayProps) => {
    ...
    const render_chords = useCallback(() => {
        ...
        Array.prototype.map.call(songxmlRef.current.querySelectorAll('.chord'), (el) => {
            const chord = transRef.current.getNewChord(el.dataset.chord, transposeObj.get_total_delta(), transposeObj.key, transposeObj.is_minor);
    ...
    const refresh_songxml = useCallback(() => {
        ...
        const showChordsVal = !!(no_chords ? false : /<chord>/i.test(contentStr) && display_chords);
